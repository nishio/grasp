...
- ユーザが Markdown folder、またはこの repo の `wiki/` を調べたい場合は、必要に応じて task-local store へ read-only mirror として index する:
  ...
  最小 Markdown mirror は file stem を title とし、`[[...]]` と `#tag` を grasp 内 edge にする。バックティックのプレーン名（親 llm-wiki への cross-wiki 参照）は edge にしない。既存 Markdown folder へは書き戻さない。frontmatter title / aliases / Obsidian block refs はまだ未実装。
