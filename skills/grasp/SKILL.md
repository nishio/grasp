...
| `import-log-records <folder>` | 既存 journal に対し、Markdown log sections / `type: log-entry` files から未記録の `log_entry_import` records だけを追記する |
| `log-records` | JSONL journal の `log_entry_import` records を SQLite なしで検索・一覧する event-stream surface。`--query`（空白 term AND）/ `--subject` / `--op` / `--source-path` / `--since` / `--until` あり。record は `subjects[]` と later same-subject events を返す |
