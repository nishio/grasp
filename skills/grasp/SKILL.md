...
| `revert-plan <event-id>` | SQLite event stream から read-only rollback plan を返す。`--scope log-batch` は anchor event を含む file-back 風の作業単位を、前後の `log_append` 境界から推定し、`revert-events` に渡す candidate event ids / reverse order / revertible を返す。mutation なし |
