...
| `adopt-markdown <folder>` | 既存 Markdown wiki を store + JSONL journal に採用する authoring fast-path 入口。log page は `log_entry_import` record にも split する |
| `import-log-records <folder>` | 既存 journal に対し、Markdown log sections から未記録の `log_entry_import` records だけを追記する |
