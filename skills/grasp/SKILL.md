...
- この repo の file-back dogfood では、gitignored store `.grasp/file-back.sqlite`、project `grasp-wiki`、output `wiki` を既定の組として使い、通常編集は `--no-journal` path にする。`wiki.grasp/events.jsonl` は transition 中の compatibility/audit artifact で、明示的に audit journal も更新したい時だけ `--journal wiki.grasp/events.jsonl` を渡す。通常編集の authority は SQLite store 側に置き、write 前に `git fetch origin main` と `python3 scripts/check_file_back_preflight.py`（no-journal default）を通して remote 分岐なし / wiki dirty なし / `write-status --no-journal --strict` / SQLite authority projection を検査し、write 後に `python3 scripts/check_file_back_postwrite.py`（no-journal default）で strict status / projection policy / wiki lint / diff whitespace をまとめて検査する。ship loop では `python3 scripts/check_file_back_runbook.py` で runbook が no-journal default から drift していないことも検査する。grasp alpha が安全に表現できない変更だけ fallback する。
