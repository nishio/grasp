...
| `log-records` | JSONL journal の `log_entry_import` records を SQLite なしで検索・一覧する event-stream surface。`--query`（空白 term AND）/ `--op` / `--source-path` / `--since` / `--until` あり |
| `history <query>` | `read <page>` と分けて log event stream を検索する薄い surface。subject extraction 前は `log-records --query` 相当の空白 term AND |
