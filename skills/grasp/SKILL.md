...
- `grasp import --cosense <json>` で Cosense JSON export、`grasp import --markdown <folder>` で read-only Markdown mirror、`grasp import-forest <wikis.yaml>` で registry 配下の複数 Markdown wiki を import する。authoring fast path では `grasp adopt-markdown <folder> --journal <events.jsonl>`、`grasp export-markdown --output <folder> --check`、`grasp append-section ... --output <folder>`、`grasp append-log ... --output <folder>`、`grasp write-page ... --from-file <file> --output <folder>`、`grasp write-status --output <folder>`、`grasp write-diff --output <folder>`、`grasp revert-event <event-id> --output <folder>`、`grasp replay-journal --journal <events.jsonl> --output <folder> --check` を使う。write 系は Markdown-backed project の unique handle に限る alpha surface。`write-page` は title / aliases / source path を変えず本文行だけ全置換する。`revert-event` は append event の inserted lines が現在も page tail にある時、または page_update の current lines が一致する時だけ動く。`replay-journal` は `page_create` / `page_update` / `section_append` / `log_append` / `event_revert` の strict replay に対応。rename / source-path 変更 / 汎用 revert はまだ無い。以降は sub-second。別パスは `--store` / `$GRASP_STORE`、別 home は `$GRASP_HOME`。
