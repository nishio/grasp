...
- この repo の file-back dogfood では、gitignored store `.grasp/file-back.sqlite`、project `grasp-wiki`、compatibility/audit journal `wiki.grasp/events.jsonl`、output `wiki` を既定の組として使う。`wiki.grasp/events.jsonl` がある時は direct Markdown patch でなく grasp write first。通常編集の authority は SQLite store 側に置き、write 前に `git fetch origin main` と `python3 scripts/check_file_back_preflight.py` を通して remote 分岐なし / wiki・journal dirty なし / `write-status --strict` / SQLite authority projection を検査し、write 後に `python3 scripts/check_file_back_postwrite.py` で strict status / projection policy / wiki lint / diff whitespace をまとめて検査する。`--no-journal` cutover 検証時は同じ checker に `--no-journal` を付け、`write-status --no-journal --strict` と dirty `wiki` path のみを guard する。grasp alpha が安全に表現できない変更だけ fallback する。
