...
## nishio 採否（2026-06-23）

findings 表は feedback の faithful record。各候補への nishio の adjudication（live status は [[grasp-backlog]]）:

- **read --related-snippets**: 採用方向。**実際の Cosense UI も related の先頭 5 行程度を表示**するので default snippet = 先頭 ~5 行（Cosense parity）。
- **line-id 冗長**: agree（別名化）。
- **`--strip-decoration`**: **却下**。decoration は noise でない — `[nishio.icon]` は **block の著者**を示し、bare image URL は **今の AI に読めなくても人間に画像提示**でき将来 AI も読む。畳んではいけない。token 削減は line-id 別名でやる。
- **`path <A> <B>`**: experimental。研究的には筋が良いが**実用性は未知**、試作の価値あり。要確定 Open Q: グラフモデル（ノード = page か、エッジ = materialize 済み internal-link edges か）。
- **backlinks ランク（Tier 4-2）**: agree（既に views ランク済み、未済は finer weighting）。
- **近傍クラスタリング `--cluster`**: **却下**。クラスタリングは **AI がやるべき**（AI の方が賢い）。CLI 側でやるなら embeddings 導入後の雑な embedding クラスタリング程度。そもそも **100+ リンクのハブは rare case**。

却下2件（`--strip-decoration` / `--cluster`）は cost 軸の「fidelity を捨てない」「縮約は AI 層」という [[ai-consumer-cost-and-trust]] の方針を nishio が確定させた形。
...
2026-06-23 23:10: literal 0件時の normalized fallback（NFKC query 正規化＋長音除去）を追加。`ﾕｰｻﾞﾃｽﾄ` が `ユーザテスト` / `ユーザーテスト` 行に hit し、`match_mode: "normalized"` を返す。大規模 store での完全なかな/カナ正規化 index、OR、related 空結果 contract は未実装。current facts は [[grasp-v1-implemented]]、残課題は [[grasp-backlog]]。
