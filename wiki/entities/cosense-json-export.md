...
実装後の補正: broad bracket 分類の「残り」は code/list/decoration notation も含む。`grasp` MVP parser は `wanted` noise を減らすため、inline backtick 内、ASCII index 風 `xs[i]` / `func()[0]`、数字のみ `[1]`、連続 `*`/`-`/`_` 装飾 `[** x]` を link から除外する（[[grasp-cli-mvp]]）。
...
- `grasp` MVP の strict parser では 120693 edges / red link 41750。上記 133022 / 61613 / 45703 は broad bracket 分類の値。
