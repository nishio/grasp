...
- user-level `file-back` skill は grasp-backed LLM Wiki repository で `wiki.grasp/events.jsonl` がある場合、direct Markdown patch より先に grasp write path を試す。repo-local `/next` command と AGENTS/CLAUDE も、`.grasp/file-back.sqlite`（gitignored store） + project `grasp-wiki` + compatibility/audit journal `wiki.grasp/events.jsonl` + output `wiki` を既定の file-back dogfood path として扱う（`1.7.22`）。通常編集の authority は SQLite store 側に置く。この repo では `scripts/check_projection_policy.py` が `export-markdown --json --check` の `projection_policy` を検査し、SQLite authority / stored lines base / git-tracked projection role / clean projection を file-back / ship loop の guard にする。`scripts/check_file_back_preflight.py` はその projection guard に加え、`origin/main` の差分なし、wiki/journal dirty なし、`write-status --strict` clean を write 前にまとめて確認する。`scripts/check_file_back_postwrite.py` は write 後に strict status / projection policy / wiki lint / whitespace diff check をまとめて確認する。
...
- current public compatibility version は `1.8.12`。release / store compatibility の履歴と bump rule は [[history]]。
...
- `grasp.journal` は LLM Wiki infra fast path の event JSONL contract module。schema v1、event types `page_create` / `page_update` / `section_append` / `page_rename` / `log_append` / `log_entry_import` / `projection_export` / `event_revert`、canonical JSONL encode / append / read validation を持つ。`adopt-markdown` は initial `page_create` events と log page / `type: log-entry` file 由来の `log_entry_import` records を SQLite `events` に insert してから legacy JSONL に append する。`import-log-records` は missing / updated `log_entry_import` records を SQLite `events` に insert してから legacy JSONL に append する。`log-records` / `history` は SQLite `log_entry_import` rows を優先し、無ければ legacy JSONL `log_entry_import` records を event stream として読む。`log_entry_import` payload は `record_identity`, `content_fingerprint`, `record_format`, `subjects[]`, `explicit_subjects[]`, `heuristic_subjects[]`, `subject_source`, `sources[]` を持てる。read 時は同一 record の versions に `record_version`, `record_version_count`, `superseded_by`, `supersedes` を付け、既定では superseded version を隠す。古い payload でも read 時に `[[wikilink]]` / Markdown path から subjects を再推定する。`history` は subject match で record を引き、same-subject later events を同梱する。`append-section` / `append-log` は SQLite index 更新と同じ transaction で `section_append` / `log_append` events を insert し、Markdown projection を export する。`write-page --create` は `page_create`、既存 page の `write-page` は `page_update` event を同じ transaction で insert する。`rename-page` は rename state update と同じ transaction で `page_rename` event を insert する。これらの write commands は既定では compatibility JSONL append も続けるが、`--no-journal` では SQLite events と projection だけを更新する。`revert-event` は SQLite-sourced target の state revert と `event_revert` insert を同じ transaction に畳み、既定では legacy JSONL append も続ける。projection export failure rollback は page_create/append/log/page_update/page_rename を取り消し、state revert と SQLite `event_revert` insert を同じ transaction に畳む。journal ありなら rollback event も legacy JSONL に append し、`--no-journal` なら JSONL には触れない。`replay-journal` は `page_create` / `page_update` / `page_rename` / append / `log_entry_import` / revert の strict replay を行い、page state guard は `line_index` + `text` 一致を要求するが line_id drift では止めない。実 git history replay corpus は単発 rename、単発 page create + multi-page update、source digest policy の既存6ページ横断 update、表駆動の連続 commit page_update replay（`3eaab75`→`3605e05`, `7360053`→`8278069`）、create を含む連続 replay（`0db1449`→`a07f1af`）、rename を含む continuous table replay（`d4e4c39`）、event_revert を含む continuous table replay（`0db1449` の page_create revert）を含む。さらに `write-page --create` で path stem と title が異なる title==H1 page を作り、`rename-page` 後に fresh `import --markdown` しても旧 title alias が残る regression test を含む。general revert は未実装。
...
- SQLite events table: schema v8 で legacy JSONL event schema を store 内に保持できる `events` table を追加した。`SQLiteStore.import_journal_events()` は JSONL path または event dict list を読み、selected/project filter と duplicate skip 付きで import する。`SQLiteStore.events()` / `event_count()` は selected project、明示 project、event type で query する。`adopt-markdown` initial events、`write-page` / `write-page --create`、`append-section` / `append-log`、`rename-page`、SQLite-sourced `revert-event`、projection export failure rollback は SQLite `events` rows を書く。`import-log-records` は new / updated `log_entry_import` rows を SQLite `events` に insert し、`log-records` / `history` は SQLite `log_entry_import` rows を優先して読む。`write-status` は selected project の SQLite event count / last event と、legacy JSONL journal との event stream match/mismatch を表示する。`--no-journal` 付き write/status は compatibility journal を active dependency にしない。`write-diff` は `1.8.8` で削除済み。
