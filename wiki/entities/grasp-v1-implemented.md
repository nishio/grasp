...
- current public compatibility version は `1.8.9`。release / store compatibility の履歴と bump rule は [[history]]。
...
| `write-status` | authoring alpha の recovery surface。Markdown projection を `--check` し、journal path の存在、JSONL event count、`journal_log_record_count`、JSONL last event、SQLite `events` count、SQLite last event を返す。journal がある場合は primary log page を journal 由来 projection と比較し、direct Markdown import で通常 projection が clean になっていても log が journal から stale なら `journal_log_stale=true` と `journal_log_changed_files` を返す。`--strict` は projection dirty / journal missing / stale log / log regeneration error で exit 1 にする。ただし journal-regenerated log projection が clean な場合、stored log lines と generated log の差だけでは strict failure にしない。SQLite events と JSONL の count mismatch はまだ strict failure にしない |
