...
- current public compatibility version は `1.8.11`。release / store compatibility の履歴と bump rule は [[history]]。
...
- SQLite events table: schema v8 で legacy JSONL event schema を store 内に保持できる `events` table を追加した。`SQLiteStore.import_journal_events()` は JSONL path または event dict list を読み、selected/project filter と duplicate skip 付きで import する。`SQLiteStore.events()` / `event_count()` は selected project、明示 project、event type で query する。`adopt-markdown` initial events、`write-page` / `write-page --create`、`append-section` / `append-log`、`rename-page`、SQLite-sourced `revert-event`、projection export failure rollback は SQLite `events` rows を書く。`import-log-records` は new / updated `log_entry_import` rows を SQLite `events` に insert し、`log-records` / `history` は SQLite `log_entry_import` rows を優先して読む。`write-status` は selected project の SQLite event count / last event と、legacy JSONL journal との event stream match/mismatch を表示する。`write-diff` は `1.8.8` で削除済み。
