...
- current public compatibility version は `1.7.14`。release / store compatibility の履歴と bump rule は [[history]]。
...
| `revert-event <event-id>` | `section_append` / `log_append` / `page_update` journal event を対象にする recovery surface。append/log は inserted lines が現在も target page 末尾に完全一致する時だけ削除する。page_update は current lines が一致する時だけ previous lines へ戻す。削除/復元後に `event_revert` を journal に append し、Markdown projection を export する |
| `replay-journal` | authoring alpha の recovery surface。SQLite を読まず、JSONL journal の `page_create` / `page_update` / `section_append` / `log_append` / `event_revert` を strict replay して Markdown projection を reconstruct / compare / write する。multiple project journal では `--project` が必要 |
