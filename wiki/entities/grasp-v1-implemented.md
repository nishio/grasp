...
- current public compatibility version は `1.7.39`。release / store compatibility の履歴と bump rule は [[history]]。
...
- SQLite write substrate: `connect_sqlite_store(..., for_write=True)` は WAL / `busy_timeout` / `synchronous=NORMAL` を設定し、`sqlite_write_transaction()` は `BEGIN IMMEDIATE` で write lock を取り commit/rollback する。CLI は store 更新系 command を write-configured connection で開く。ただし events table と state+event 1 transaction command migration は未実装。
