...
- current public compatibility version は `1.8.2`。release / store compatibility の履歴と bump rule は [[history]]。
...
| `rename-page <target> <new-title>`（alias: `rename`） | Markdown-backed project の page id を保ったまま title と optional source path を変更する authoring alpha。target は unique handle / page-id / path。旧 title と旧 file stem を alias handle に残し、incoming `[[旧名]]` の surface text は書き換えず edge resolution を更新する。first H1 が旧 title と一致する時だけ同じ line_id のまま `# <new-title>` に更新する。path-derived id / first H1 から identity が推論できても、title / current file stem から導出できない旧名 alias がある場合は projection に `id` / `title` / `aliases` frontmatter を生成・更新するため direct re-import 後も page id と旧名 alias が残る。既存 frontmatter がある時は `type` / `summary` / `sources` などを保持する。`page_rename` journal event、projection file rename、replay/revert に対応し、JSON result は `event_type=page_rename` を返す |
...
- SQLite events table: schema v8 で legacy JSONL event schema を store 内に保持できる `events` table を追加した。`SQLiteStore.import_journal_events()` は JSONL path または event dict list を読み、selected/project filter と duplicate skip 付きで import する。`SQLiteStore.events()` / `event_count()` は selected project、明示 project、event type で query する。`write-page` / `write-page --create` と `append-section` / `append-log` は state update と SQLite `events` row insert を1 transaction に畳んだ。`rename-page` / `revert-event` / projection failure rollback / history surfaces はまだ legacy JSONL-first path を含む。
