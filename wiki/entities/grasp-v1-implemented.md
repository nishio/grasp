...
- current public compatibility version は `1.7.37`。release / store compatibility の履歴と bump rule は [[history]]。
...
- Markdown line text は原文のまま保存する。frontmatter も本文行として残す。projection が identity 用 `id` / `title` / `aliases` frontmatter を必要とする時は、既存 frontmatter の任意 metadata を保持し、projection 管理 fields だけを差し替える。
