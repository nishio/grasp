...
- current public compatibility version は `1.7.23`。release / store compatibility の履歴と bump rule は [[history]]。
...
| `write-status` | authoring alpha の recovery surface。Markdown projection を `--check` し、journal path の存在、event count、last event を返す。journal がある場合は primary log page を journal 由来 projection と比較し、direct Markdown import で通常 projection が clean になっていても log が journal から stale なら `journal_log_stale=true` と `journal_log_changed_files` を返す |
