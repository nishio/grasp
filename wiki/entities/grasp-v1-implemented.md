...
- current public compatibility version は `1.8.1`。release / store compatibility の履歴と bump rule は [[history]]。
...
- SQLite events table: schema v8 で legacy JSONL event schema を store 内に保持できる `events` table を追加した。`SQLiteStore.import_journal_events()` は JSONL path または event dict list を読み、selected/project filter と duplicate skip 付きで import する。`SQLiteStore.events()` / `event_count()` は selected project、明示 project、event type で query する。`write-page` / `write-page --create` は state update と SQLite `events` row insert を1 transaction に畳んだ。`append-section` / `append-log` / `rename-page` / `revert-event` / projection failure rollback / history surfaces はまだ legacy JSONL-first path を含む。
