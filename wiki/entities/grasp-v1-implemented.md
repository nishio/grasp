...
- current public compatibility version は `1.8.6`。release / store compatibility の履歴と bump rule は [[history]]。
...
| `adopt-markdown <folder>` | 既存 Markdown wiki を SQLite materialized index に import し、各 page を `page_create` event として JSONL journal に記録する。log page sections と frontmatter `type: log-entry` files を `log_entry_import` record event へ split する。section record は body の `[[wikilink]]` と Markdown path から `subjects[]` を推定し、file record は frontmatter `date` / `timestamp`, `op`, `summary`, `subjects` / `pages`, `sources` を読む。既存 journal は `--replace-journal` なしでは上書きしない。これは native authority への adoption surface |
...
- SQLite events table: schema v8 で legacy JSONL event schema を store 内に保持できる `events` table を追加した。`SQLiteStore.import_journal_events()` は JSONL path または event dict list を読み、selected/project filter と duplicate skip 付きで import する。`SQLiteStore.events()` / `event_count()` は selected project、明示 project、event type で query する。`write-page` / `write-page --create`、`append-section` / `append-log`、`rename-page`、SQLite-sourced `revert-event` は state update と SQLite `events` row insert を1 transaction に畳んだ。`import-log-records` は new / updated `log_entry_import` rows を SQLite `events` に insert し、`log-records` / `history` は SQLite `log_entry_import` rows を優先して読む。`write-status` は selected project の SQLite event count / last event を表示する。projection failure rollback / adopt-markdown initial events / write-diff はまだ legacy JSONL-first path を含む。
