...
### 2026-06-30: Open Q #1/#2 は cutover decision の下流（合成）

未決の Open Q #1（pairing guard worktree-aware 化 / projection defer）と #2（direct-patch + remote-merge 正規化 / append-only projection の自動 merge）は、独立な2問でなく **同一の上位判断「SSoT をどこに置くか = mode1 ↔ mode2 cutover」（[[adoption-trust-gradient]]）の下流**。今の contention の痛みは、store（gitignored = local scaffolding）と git-tracked Markdown の **両方が authority として振る舞う**過渡状態から出ている（[[adoption-trust-gradient]]: 同一コンテンツに両モード同時 on 不可）。

- **mode2（store=SSoT）に倒すと**: Q#1 は (ii) defer-projection が自然（`write-page` / `append-log --defer-projection` 1.8.72 で半実装済み、projection の一時 stale が許容される）。Q#2 の append-only merge は「store events から regenerate」で解け、direct-patch は cutover までの暫定橋になる。
- **mode1（Markdown=SSoT）に留めると**: Q#2 の direct-patch + remote-merge が逃げ道でなく本来の authoring 経路、store は throwaway index、log merge は純 git 層の merge-driver 問題。Q#1 の projection defer は moot。

∴ **Q#1/#2 を個別に詰める前に cutover 方向を先に固定するのが筋**。leverage は Q#1 (ii) defer-projection 方向で mode2 へ寄せること（substrate goal「並行 write 安全」に直撃、実装も軽い）。これが決まれば Q#2 は下流として自動的に道が付く。本 synthesis は分析であって決定ではない（cutover を倒す判断は owner=nishio）。
