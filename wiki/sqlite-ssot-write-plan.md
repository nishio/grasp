...
With projection policy, no-journal default preflight/postwrite checks, command-level `--no-journal`, and repo runbook cutover now in place, the next slice is guarded no-journal dogfood plus the generated Markdown backup/review policy only if the dogfood exposes a concrete gap.

1. Perform the next three wiki file-backs using `git fetch origin main` + `scripts/check_file_back_preflight.py`, grasp write with `--no-journal --output wiki`, then `scripts/check_file_back_postwrite.py`.
