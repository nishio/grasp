...
With projection policy, no-journal default preflight/postwrite checks, command-level `--no-journal`, repo runbook cutover, a 3-file-back no-journal dogfood streak, tracked JSONL retirement, SQLite-sourced `--regenerate-log`, postwrite semantic log guard, native `write-status` semantic log status, and machine-readable projection rollback diagnostics in place, the next slice is the remaining SQLite authority general revert policy. Generated Markdown backup/review policy should still be clarified only if a concrete recovery or review gap appears.

1. Define whether "general revert" should mean multi-event rollback, dependency-aware revert planning, or only richer dry-run/status diagnostics for existing event-specific reverts.
