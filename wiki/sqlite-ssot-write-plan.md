...
With projection policy, no-journal default preflight/postwrite checks, command-level `--no-journal`, repo runbook cutover, a 3-file-back no-journal dogfood streak, tracked JSONL retirement, SQLite-sourced `--regenerate-log`, postwrite semantic log guard, native `write-status` semantic log status, machine-readable projection rollback diagnostics, `revert-event --dry-run` planning, `revert-event --include-dependents` execution, `revert-events` explicit multi-event rollback, and `revert-plan --scope log-batch` inferred work-unit planning in place, the next slice is broader inferred/general revert planning for histories that do not have usable log-batch boundaries. Generated Markdown backup/review policy should still be clarified only if a concrete recovery or review gap appears.

1. Identify the next real revert gap that cannot be handled by explicit event ids, active same-page dependencies, or log-batch boundaries, preferably from dogfood or the existing git history replay corpus.
