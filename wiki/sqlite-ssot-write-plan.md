...
This does **not** yet make every authority boundary final. `sync`, `acquire`, generated Markdown backup/review policy, broader native event-derived semantic page projection, and inferred/general revert planning beyond explicitly selected events or active same-page dependents still need migration work.
...
With projection policy, no-journal default preflight/postwrite checks, command-level `--no-journal`, repo runbook cutover, a 3-file-back no-journal dogfood streak, tracked JSONL retirement, SQLite-sourced `--regenerate-log`, postwrite semantic log guard, native `write-status` semantic log status, machine-readable projection rollback diagnostics, `revert-event --dry-run` planning, `revert-event --include-dependents` execution, and `revert-events` explicit multi-event rollback in place, the next slice is broader inferred/general revert planning beyond explicitly selected events and active same-page dependencies. Generated Markdown backup/review policy should still be clarified only if a concrete recovery or review gap appears.

1. Identify the next real revert gap that needs inferred dependency planning rather than explicit event ids, preferably from dogfood or the existing git history replay corpus.
