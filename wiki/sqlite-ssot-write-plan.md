...
| 6. File-back cutover | Daily wiki edits use the new authority path | Update file-back skill / repo commands to call canonical SQLite write first, then export Markdown, lint, and commit projection | In progress: repo-local and local user-level file-back instructions now run executable preflight and postwrite checks around the grasp write path; the repo-local checkers also have `--no-journal` mode; done when three consecutive file-backs land through SQLite SSoT without direct Markdown patch fallback |
| 7. Retire JSONL authority | Old alpha artifacts cannot mislead implementers | Mark `wiki.grasp/events.jsonl` as compatibility/audit artifact or remove it from the active path; update docs/AGENTS when the command surface changes | In progress: active runbooks now label `events.jsonl` compatibility/audit and SQLite-side authority. Write commands, `write-status`, and repo-local guard scripts now support `--no-journal`; repo file-back still passes the compatibility journal until the guarded dogfood streak or an explicit cutover decision says otherwise |

## Immediate Next Slice

With projection policy, preflight, postwrite checks, command-level `--no-journal`, and checker-level `--no-journal` now in place, the next slice is guarded file-back dogfood plus the cutover decision for this repository's own file-back path.
