...
`1.8.73` turns the parallel file-back incident's recovery lesson into guard output. When repo-local preflight/write-start stops on dirty projection/worktree, branch/HEAD movement, semantic log drift, SQLite event advancement after preflight, store/output pair mismatch, reused session_id, or active lock, stderr now includes a `recovery ladder:`. The ladder starts with `activity --limit 20` ownership inspection and routes agents toward folding into the owner branch, isolated direct-patch PR plus pending reconcile, clean owner-worktree reconcile, preflight rerun, or waiting. This is not automated reconcile yet; it is the first executable surface for [[parallel-agent-substrate-goal]]'s detection / guardrail / recovery-ladder dogfood.
