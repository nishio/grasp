...
| 7. Retire JSONL authority | Old alpha artifacts cannot mislead implementers | Mark `wiki.grasp/events.jsonl` as compatibility/audit artifact or remove it from the active path; update docs/AGENTS when the command surface changes | In progress: active runbooks now label `events.jsonl` compatibility/audit and SQLite-side authority. Write commands and `write-status` now support `--no-journal`; repo file-back still passes the compatibility journal until the guarded dogfood streak or an explicit cutover decision says otherwise |

## Immediate Next Slice

With projection policy, preflight, postwrite checks, and command-level `--no-journal` now in place, the next slice is guarded file-back dogfood plus the cutover decision for this repository's own file-back path.
