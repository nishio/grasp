...
This does **not** yet make every authority boundary final. `sync`, `acquire`, generated Markdown backup/review policy, broader native event-derived semantic page projection, and semantic multi-page work-unit inference beyond log-batch, same-page dependency, explicit event-window, or time-burst boundaries still need migration work.
...
With projection policy, no-journal default preflight/postwrite checks, command-level `--no-journal`, repo runbook cutover, a 3-file-back no-journal dogfood streak, tracked JSONL retirement, SQLite-sourced `--regenerate-log`, postwrite semantic log guard, native `write-status` semantic log status, machine-readable projection rollback diagnostics, `revert-event --dry-run` planning, `revert-event --include-dependents` execution, `revert-events` explicit multi-event rollback, and `revert-plan` planning for log-batch, same-page-dependents, explicit event-window, and explicit time-burst scopes in place, the next slice is semantic multi-page work-unit inference for histories that lack usable log-batch boundaries, cannot be described by same-page dependencies, and should not be bounded by a counted event window or purely temporal gap. Generated Markdown backup/review policy should still be clarified only if a concrete recovery or review gap appears.

1. Identify the next real multi-page revert gap that cannot be handled by explicit event ids, active same-page dependencies, log-batch boundaries, an explicit event-window count, or an explicit time gap, preferably from dogfood or the existing git history replay corpus.
