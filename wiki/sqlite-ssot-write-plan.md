...
With projection policy, no-journal default preflight/postwrite checks, command-level `--no-journal`, repo runbook cutover, a 3-file-back no-journal dogfood streak, tracked JSONL retirement, SQLite-sourced `--regenerate-log`, postwrite semantic log guard, native `write-status` semantic log status, machine-readable projection rollback diagnostics, `revert-event --dry-run` planning, and `revert-event --include-dependents` execution in place, the next slice is broader SQLite authority general revert policy beyond the concrete active same-page dependency case. Generated Markdown backup/review policy should still be clarified only if a concrete recovery or review gap appears.

1. Identify the next real revert gap outside later active same-page reversible dependencies, preferably from dogfood or the existing git history replay corpus.
