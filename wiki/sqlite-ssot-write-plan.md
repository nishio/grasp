...
| 6. File-back cutover | Daily wiki edits use the new authority path | Update file-back skill / repo commands to call canonical SQLite write first, then export Markdown, lint, and commit projection | In progress: repo-local runbooks now default to `--no-journal` guarded file-back and the runbook checker guards against documentation drift. Remaining proof is a short dogfood streak through the no-journal default without direct Markdown patch fallback |
...
With projection policy, preflight, postwrite checks, command-level `--no-journal`, checker-level `--no-journal`, and repo runbook cutover now in place, the next slice is guarded no-journal dogfood plus the generated Markdown backup/review policy only if the dogfood exposes a concrete gap.

1. Perform the next three wiki file-backs using `git fetch origin main` + `scripts/check_file_back_preflight.py --no-journal`, grasp write with `--no-journal --output wiki`, then `scripts/check_file_back_postwrite.py --no-journal`.
