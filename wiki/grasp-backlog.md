...
安定マイルストーンの読み替え（2026-06-30）: ここでの stable は read v1 ではなく、mode2 / write-authoring を安心して dogfood stable に近づける gate として扱う。優先順は、(1) `scripts/benchmark_claim_retry_throughput.py` を larger N / 複数 think time / file-back 風 workload に拡張し、`lost=0` / strict green / active claim overlap 0 / completed throughput ratio / surviving marker throughput ratio / p95 claim wait を同じ表で出す、(2) owner が throughput 下限と p95 wait 上限などの cutover 閾値を決める、(3) real dogfood で append / merge surface の要否を判定する、(4) mode2 中の Markdown 直接編集 policy（reject / adopt / merge、基本は Markdown read-only projection）を guard / runbook に落とす、(5) stable line identity（`line.id` と `line_index` の分離、reimport diff の id 引き継ぎ）と、git diff of Markdown に依存しない review/recovery evidence を揃える。実例なしに汎用 merge / queue を先に作らない。
...
- **【Codex handoff】mode2 並行編集 dogfood が出した実装タスク**（証跡 [[mode2-parallel-edit-stress-2026-06-30]]）。2026-06-30 に throwaway mode2 store を2プロセスで並行 loop 編集した結果、現状の前提を直す具体作業が出た。(P0) は cutover の前提条件:
  ...
  - 受け入れ条件: 「2プロセス×N iteration の hot-page 奪い合いで lost-update 0」を回帰試験 `tests/` に固定する第一段は P0b/P1a/P1b claim retry で進み、2026-06-30 Codex follow-up で軽量 subprocess regression（2 worker × 4 iteration、同一 page、全 marker 生存、projection export 後 strict green）として固定済み。同 follow-up で `scripts/benchmark_claim_retry_throughput.py` も追加し、同条件（2 worker × 4 iteration、think 0.02s）の測定では無協調が 8 write 中 4 lost / strict failure、claim_retry が 8/8 生存 / strict green、completed write throughput は無協調比 0.322、surviving marker throughput は 0.645 だった。残る cutover gate は、この overhead を許容する閾値を決めて broader / real dogfood で再測定すること、append 系 merge surface が必要かを real dogfood で見極めること（[[adoption-trust-gradient]] の「信頼を測る指標」）。次の実験は単に lost=0 を再確認するのではなく、`scripts/benchmark_claim_retry_throughput.py` を larger N / 複数 think time / file-back 風の read→write-page→append-log→projection workload へ広げ、最低限 `lost=0`、strict green、active claim overlap 0、completed throughput ratio、surviving marker throughput ratio、p95 claim wait を同じ表で出す。mode2 cutover の可否は、その測定結果に owner が閾値（例: surviving throughput の下限、p95 wait の上限）を置いてから判断する。
