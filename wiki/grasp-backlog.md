...
- Obsidian block refs / heading anchors の細部: schema `10` で `target_fragment` / `target_line_id` は実装済み。`[[Page#Heading]]` / `[[Page#^block-id]]` と相対 Markdown link `[label](Page.md#Heading)` / `[label](Page.md#^block-id)` は target page への edge になり、unique target page 内で target line が見つかれば `target_line_id` も保持する。resolved local-only anchor `[label](#heading)` / `[[#Heading]]` は self-page line edge になる。残件は Obsidian / GitHub slug 互換の finer fidelity と、必要が具体化した時の Markdown write-alpha 由来 edge への fragment 解決。
