...
安定マイルストーンの読み替え（2026-07-02 更新）: ここでの stable は read v1 ではなく、mode2 / write-authoring を安心して dogfood stable に近づける gate として扱う。2026-07-02 時点では、(1) lost-update correctness gate は P0b/P1a/P1b と軽量回帰で green、broader synthetic でも hot-page / file-back 全6条件 50/50 生存・page/log lost 0・strict green・claim overlap 0、(2) cutover gate の初期 owner threshold は `--min-surviving-throughput-ratio 0.70` / `--max-p95-claim-wait-seconds 0.75` とし、`scripts/benchmark_claim_retry_throughput.py --profile cutover` の default に固定する、(3) file-back synthetic では append-log 喪失は出ていないので append/merge surface や queue は real dogfood の具体 workflow で必要が出るまで作らない、(4) mode2 中の Markdown 直接編集は `scripts/check_mode2_markdown_readonly.py` と runbook で既定 reject / 明示 reconcile に固定済み、(5) Codex が閾値判断なしで進められる残作業は stable line identity の編集面（Cosense/hosted sync external line id、または real dogfood で必要が出た場合の explicit split/merge surface）に寄せる。複数行 / 範囲 edit surface、duplicate text line を自動同一視しない policy、unique exact moved line の id 維持、split / merge / 大幅編集を自動同一視しない `line_identity` policy surface は 2026-07-01 に実装済み。git diff of Markdown に依存しない review/recovery evidence の最小 regression は 2026-07-01 に追加済み: file-back 風の `page_update` + `log_append` を同一 `--session-id` で束ね、別 session の update を混ぜず、`revert-plan --scope session` が projection / SQLite events を変えずに candidate / reverse order / suggested `revert-events` args を返すことを固定した。残る review/recovery backlog は、既存 scope（log-batch / subject-log / log-page-subjects / content-subjects / version-bump / same-page / event-window / time-burst / session）で説明できない実 gap、または generated Markdown backup/review policy が具体化した時に限る。実例なしに汎用 merge / queue を先に作らない。

2026-07-02 human policy update: 次に証明する価値は P3（AI author / file-back agent）の **安心して使えること**。便利な retrieval demo や外向け acquisition より先に、通常の file-back が SQLite SSoT の grasp write path だけで閉じ、Codex が direct Markdown patch に逃げたくなる理由を潰す。Markdown 直接編集は [[native-authority-markdown-projection]] の通り非常口としては許すが、発生した場合は「イレギュラー」として記録し、grasp write で表現できなかった理由と再発防止策を backlog / decision / runbook へ routing する。
...
- **【Codex handoff】mode2 並行編集 dogfood が出した実装タスク**（証跡 [[mode2-parallel-edit-stress-2026-06-30]]）。2026-06-30 に throwaway mode2 store を2プロセスで並行 loop 編集した結果、現状の前提を直す具体作業が出た。(P0) は cutover の前提条件:
  ...
  - 受け入れ条件: 「2プロセス×N iteration の hot-page 奪い合いで lost-update 0」を回帰試験 `tests/` に固定する第一段は P0b/P1a/P1b claim retry で進み、2026-06-30 Codex follow-up で軽量 subprocess regression（2 worker × 4 iteration、同一 page、全 marker 生存、projection export 後 strict green）として固定済み。同 follow-up で `scripts/benchmark_claim_retry_throughput.py` も追加し、同条件（2 worker × 4 iteration、think 0.02s）の測定では無協調が 8 write 中 4 lost / strict failure、claim_retry が 8/8 生存 / strict green、completed write throughput は無協調比 0.322、surviving marker throughput は 0.645 だった。2026-06-30 Codex follow-up 第2段で同 benchmark は larger N / 複数 think time / file-back 風の read→write-page→append-log→projection workload に拡張済みで、第3段では append-log 側の marker survival も `log_lost` として測るようにした。`--iterations 25 --think-seconds 0,0.02,0.05 --workload file-back --format table` では claim_retry が全3条件で 50/50 page marker 生存、page lost 0、log_lost 0、strict green、active claim overlap 0、p95 claim wait 0.462/0.474/0.684s。completed throughput ratio は 0.397/0.390/0.374、surviving throughput ratio は 0.794/0.780/0.747。2026-06-30 Codex follow-up 第4段で同 benchmark は owner threshold を任意指定できる `--min-surviving-throughput-ratio` / `--max-p95-claim-wait-seconds` を持った。2026-07-02 owner threshold decision で初期 fail gate は 0.70 / 0.75s に決まり、cutover profile default として使う。append 系 merge surface が必要かは real dogfood で見極める（[[adoption-trust-gradient]] の「信頼を測る指標」）。今回の file-back 風 synthetic では append-log 喪失は観測されず、generic merge / queue を先に足す根拠にはまだならない。
  - 2026-07-01 Codex follow-up で、現 HEAD に対して `--iterations 25 --think-seconds 0,0.02,0.05 --workload hot-page --workload file-back --format table` を再実行。claim_retry は hot-page / file-back の全6条件で 50/50 生存、page lost 0、log_lost 0、strict green、active claim overlap 0。file-back workload の p95 wait は 0.417/0.439/0.639s、surviving throughput ratio は 0.758/0.760/0.747。さらに `tests/test_claim_retry_throughput_benchmark.py` で matrix parsing、completed/surviving throughput ratio、lossless strict non-overlap gate、optional owner thresholds、file-back table columns を軽量 unit test として固定した。重い synthetic benchmark 本体は通常 unit suite に入れず、cutover 判断時に明示実行する。
  - 2026-07-01 schema 14 後の current HEAD で同じ broader matrix を再実行し、claim_retry は再び全6条件で 50/50 生存、page lost 0、log_lost 0、strict green、active claim overlap 0。file-back workload の p95 wait は 0.516/0.556/0.504s、surviving throughput ratio は 0.787/0.710/0.740。`scripts/benchmark_claim_retry_throughput.py --profile cutover` はこの matrix（hot-page + file-back、think 0/0.02/0.05、25 iteration/worker、table output）を既定化した。quick profile は従来の single hot-page JSON を維持する。
  - 2026-07-01 follow-up で同 benchmark は `metric_summary` / `Cutover Metric Summary` を返すようになった。これは owner 閾値を決めるための worst-case evidence（compared scenario count、最小 surviving/completed throughput ratio、最大 p95 wait、最大 overlap、合計 lost、all strict green）を集約するだけで、Codex が threshold を提案・採用するものではない。
  - 2026-07-01 follow-up で `--require-cutover-thresholds` を追加し、その後 `--min-surviving-throughput-ratio` と `--max-p95-claim-wait-seconds` の**両方**を必須に修正した。cutover CI / ship gate でこの flag を使うと、owner threshold 未指定の run は correctness が green でも `thresholds_not_set`、片側だけ指定した run は `required_thresholds_missing` で exit 1 になる。これは閾値を選ぶ機能ではなく、「閾値未決のまま stable 扱いしない」ための guard。
  - 2026-07-02 owner threshold decision: 初期 cutover fail gate は `min_surviving_throughput_ratio=0.70` / `max_p95_claim_wait_seconds=0.75` とする。理由は、直近 broader matrix の file-back worst が surviving throughput ratio 0.710、p95 wait 0.556s 付近で、correctness は lost 0 / strict green / overlap 0 を満たしているため。`0.80` / `0.50s` は観測目標として残すが、初期 fail gate にすると性能揺れで正しい実装を落としやすい。`scripts/benchmark_claim_retry_throughput.py --profile cutover` はこの owner default を使い、明示 flag は override として扱う。`--require-cutover-thresholds` は cutover profile default がある限り threshold 未決扱いにしない。
...
```text
lines(project, line_id, page_id, line_index, text, created, updated, user_id)
line_tombstones(project, line_id, page_id, line_index, text, created, updated, user_id, tombstoned_at, tombstone_reason)
```
