...
- duplicate title / alias collision の高度な解決（現状は import error）。2026-06-25 の wiki森全件 import dogfood（[[wiki-forest-markdown-import-dogfood-2026-06-25]]）では、registry 42 entries 中 37 entries は temp store に import 成功し、失敗 5 entries はすべて duplicate title / alias collision だった。次は collision diagnostics → alias collision softening → draft/source artifact 除外の順で進める。
