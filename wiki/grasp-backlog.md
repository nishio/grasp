...
- duplicate title / alias collision の高度な解決。2026-06-25 の wiki森全件 import dogfood（[[wiki-forest-markdown-import-dogfood-2026-06-25]]）では、registry 42 entries 中 37 entries は temp store に import 成功し、失敗 5 entries はすべて duplicate title / alias collision だった。structured diagnostics は実装済み。次は alias collision を単なる softening でなく identity/name 分離の問題として扱い、path を diagnostic / fallback handle に使っても page name へ混ぜない policy を決める。その後 draft/source artifact 除外を詰める。
