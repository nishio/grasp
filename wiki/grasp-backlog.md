...
- **[gap] direct-patch fallback の蓄積で store が main から遅れる → store→md projection 前に re-adopt を強制する check（[[parallel-session-file-back-contention-2026-06-28]]）**: 並行下で file-back runbook が pairing guard / 占有 working tree で通らないと、内容は main Markdown にだけ入り file-back store に入らない（direct-patch fallback）。2026-06-28 時点で store は一時 main から10ファイル遅れ、ssot-land の re-adopt で吸収されたが、これは「次の re-adopt が拾ってくれる」暗黙依存。逆向き（store→Markdown の full projection）を未取り込み diff があるまま走らせると、未 reconcile の direct-patch（他 session 分含む）が巻き戻る。未実装ガード: projection 前に「現 main Markdown のうち store 未取り込みの diff」を検出して止める / re-adopt を促す check。恒久対策は同 entity の (a) worktree-aware file-back / 遅延 projection (b) working-tree-level in-flight awareness。
- general `write`: 任意更新と edge 自動更新（felt-sense と come-from で別経路）。append-only alpha の `append-log`、full-page replacement alpha の `write-page`、新規 Markdown page 作成 alpha の `write-page --create --path <file.md>` は実装済み。`append-section` public CLI は `1.8.70` で削除済みで、既存 `section_append` event 互換だけを残す。
- **bug 候補（2026-06-28、要 Codex 確認）: `write-page` の handle 解決が `read` と非対称。** `read <short page_id>` は解決するのに `write-page <同 short page_id>` は `page not found` で失敗し、stem handle（例 `index`）なら通る。read で得た id を write にそのまま回せない。resolver を read 側と揃えるか `write-page` が short page_id を受けるか。詳細 [[ai-author-feedback-2026-06-26]] §Updates 2026-06-28。
