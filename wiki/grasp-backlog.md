...
- **【Codex handoff】mode2 並行編集 dogfood が出した実装タスク**（証跡 [[mode2-parallel-edit-stress-2026-06-30]]）。2026-06-30 に throwaway mode2 store を2プロセスで並行 loop 編集した結果、現状の前提を直す具体作業が出た。(P0) は cutover の前提条件:
  - **(P0a) projection / graph compute の性能病**: 高リンク密度 project（grasp-wiki ~60p / nishio 25,791p）で `export-markdown`（read-only `--check` 単体でも）と `write-page` が 96% CPU で数分〜未完 spin。低密度の llmwiki 743p は高速 → **link 密度に superlinear（O(edges²) 級）疑い**。Homebrew Python 3.14.5 環境。まず `export-markdown --check` を profile（cProfile / py-spy）して hot loop を特定、Python 3.14 回帰か corpus 成長か最近の変更かを `git bisect` で二分。**これが直らないと grasp-write file-back 自体が回らず mode2 dogfood も不可能**（本ページの file-back も direct patch fallback で着地）。
  - **(P0b) content-level lost-update 検出 guard**: 無協調並行 write で 50 中 24 が silent に消えても `write-status --no-journal --strict` は GREEN（整合は守るが消えた content を見ない）。SQLite events の actor/session/sequence を使い、並行 window 内で同一 page を上書きした overlapping page_update を検出する guard を新設。整合チェックとは別物。
  - **(P1a) `claim-page` を実効直列化へ格上げ**: 現状は soft lease で check→claim に TOCTOU 窓。stress では 0-loss だったが窓を踏まなかった幸運込み。compare-and-claim（同一 page に active claim があれば atomic に弾く）か、[[sqlite-write-concurrency]] の (A) write.lock / (B) compare-and-append journal を実装。
  - **(P1b) skip した編集の retry / merge**: claim back-off で編集を捨てる（~50% skip、throughput 半減＋dropped-work）。skip を abandon でなく requeue / retry、append 系は merge する経路。
  - 受け入れ条件: 上記を入れた状態で「2プロセス×N iteration の hot-page 奪い合いで lost-update 0 かつ throughput が claim なし比で大きく落ちない」を回帰試験 `tests/` に固定。この green を mode2 cutover の gate にする（[[adoption-trust-gradient]] の「信頼を測る指標」）。
