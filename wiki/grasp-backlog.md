...
- `SQLite SSoT write substrate`: `1.7.39` で canonical path helper、WAL / busy_timeout write connection、`BEGIN IMMEDIATE` transaction helperは実装済み。`1.8.0` で SQLite `events` table、既存 JSONL import helper、SQLite event query helper も実装済み。未実装は state+event 1 transaction write command migration、export-only Markdown projection、SQLite events 由来の history / write-diff / revert-event。file-back skill cutover はこの substrate と native recovery が先に通ってから。
...
- durable journal policy: 方向は [[sqlite-write-concurrency]] / [[sqlite-ssot-write-plan]] で **SQLite primary + events table** に決めた。canonical store helper、events table、JSONL import、actor/session metadata columns は実装済み。未実装は generated Markdown の backup/review policy、既存 write command の SQLite events 書き込み、SQLite events 由来の recovery surface。
