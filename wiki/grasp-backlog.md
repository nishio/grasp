...
- status / diff / rollback or revert event: append/log/page_update/page_rename alpha 用の `write-status` / `write-diff` / `revert-event` は実装済み（`1.7.12`-`1.7.15`）。未実装は replayable authority 上の general revert / rollback、projection export 失敗時の transaction boundary。
