...
- Obsidian block refs / heading anchors の line-id 対応: page-level edge は実装済み。`[[Page#Heading]]` / `[[Page#^block-id]]` と相対 Markdown link `[label](Page.md#Heading)` / `[label](Page.md#^block-id)` は target page への edge になる。未実装は anchor / block target を target `line_id` として保持する schema と、local-only anchor `[label](#heading)` / `[[#Heading]]` を self-page line target として扱う設計。
