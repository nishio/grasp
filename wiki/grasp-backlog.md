...
- `export-markdown`: stored lines preserving projection と `--check` no-op gate は実装済み（`1.7.10`）。未実装は native events からの semantic projection（page files / index projection / log event stream projection）と write 後の regeneration。
...
- journal JSONL event type contract は `grasp.journal` で固定済み（`1.7.9`）。`adopt-markdown` は `page_create` events を append する（`1.7.10`）。未実装は replayable authority と SQLite materialized index の接続、write CLI からの append。
