...
- 40+ wiki を 1 store の 40+ project namespace へ import するオーケストレーション（`wikis.yaml` を seed に `import --markdown <folder> --project <wiki-name>` を一括実行）。
- 森規模での navigation/log artifact 除外（下記2節を森に適用）。
- **raw/ 除外オプション**（性能前提。`llm-wiki-about-nishio` は md 24,968 件）。
...
`index.md` 等は通常の content page でなく navigation / generated projection として扱う（[[markdown-obsidian-indexed-mirror]]）。最小 mirror は全 `.md` を同列 import するため、親 llm-wiki 規模では `index.md` が巨大 hub になり `related` / `path` を汚す。未実装:

- navigation artifact の分類（path heuristic `index.md` / `log.md` / `forest-index.md` / `maps/` / `views/` ＋ frontmatter `role: navigation`）。
- navigation の outgoing edges を既定 content graph から除外（検索対象には残すが `related` / `path` / backlink ranking では混ぜない）。
...
`log.md` は content page でなく append-only event stream / provenance として扱う（[[markdown-obsidian-indexed-mirror]]）。設計上の主問題は log entry を巨大 page 内の section とするか、stable identity を持つ first-class record として materialize するか＝grasp では後者。さらに **log entry は現在状態の主張でなく過去の transition event**（A→B→C で `B になった` entry だけ読んで「今は B」と答えるのは誤り）。未実装:
...
- log artifact の outgoing links を既定 content graph から除外（search / provenance query には残す）。
