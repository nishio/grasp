...
- weak 接続の **cross-wiki spread ranking** は `cross-project-spread <title>` として最小実装済み。次は spread の dogfood と、whole-store retrieval / first-class cross-project edge への接続。
...
- **cross-project link を materialize**: `cross-project-spread` は schema-compatible な観測 pre-step として実装済みだが、edge ではない。次は `edges` に `target_project` / `link_kind`（internal / cross-semantic / cross-icon / cross-root）を追加、`source_project` へ rename。`CrossProjectLink`（`raw/project/title/target_class`）を流し、`raw` 保存で slash-in-title を再解決可能に。
