...
- durable journal policy: 方向は [[sqlite-write-concurrency]] / [[sqlite-ssot-write-plan]] で **SQLite primary + events table** に決めた。canonical store helper、events table、JSONL import、actor/session metadata columns、`write-page` / `append-section` / `append-log` / `rename-page` / SQLite-sourced `revert-event` の SQLite events 書き込み、`write-status` の SQLite event summary は実装済み。未実装は generated Markdown の backup/review policy、projection failure rollback の SQLite events 書き込み、SQLite events 由来の `write-diff` / `history` recovery surface。
