...
- **progressive / lazy import**: 2026-06-30 に `refresh_edge_resolutions` の急性性能病は修正し、同日 follow-up で no-op re-import の manifest-first fast path も実装した。既存 manifest の source type / exclude dirs / file set / content hash が完全一致する時は `MarkdownMirror.from_folder()` を呼ばず、3000p/54000 edges synthetic no-op は約 0.82s。残る本体は初回 / changed-file re-import が依然として全 file parse / 全 edge build から入ること。大きな corpus / 初回 onboarding では「最初に全部を完全 materialize する」より、manifest-first catalog、本文 parse の page-on-demand / chunked background hydration、graph derivative の stale/incomplete flag、query が必要な近傍から徐々に良くする design を検討する。成功条件は初回 usable latency を小さくし、後続 command が incomplete graph を誤って complete と表示しないこと。
