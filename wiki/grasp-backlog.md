...
- duplicate title / alias collision の高度な解決（現状は import error）。2026-06-25 の wiki森全件 import dogfood では、registry 42 entries 中 37 entries は temp store に import 成功し、失敗 5 entries はすべて duplicate title / alias collision だった。典型は draft version が同一 H1 を持つ、`_overview` / `README` / `index` のような file stem alias が複数 directory で衝突する、source/session file と canonical page が同じ alias を持つ、の3類型。次は collision を即 error で全 project import 停止にせず、path-qualified alias / artifact-draft 除外 / collision report として扱う設計が必要。
...
- 40+ wiki を 1 store の 40+ project namespace へ import するオーケストレーション（`wikis.yaml` を seed に `import --markdown <folder> --project <wiki-name>` を一括実行）。2026-06-25 手動 dogfood では import 可能 project を 1 store に並べること自体は成立した（37 projects / 2458 pages / 213k lines / 22.5k edges / 1412 unresolved）。
