...
- `export-markdown`: native store から Markdown projection（page files / index projection / log event stream projection）を生成する。
...
- journal JSONL event type contract は `grasp.journal` で固定済み（`1.7.9`）。未実装は replayable authority と SQLite materialized index の接続、`adopt-markdown` / write CLI からの append。
