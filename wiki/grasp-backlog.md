...
- `SQLite SSoT write substrate`: `1.7.39` で canonical path helper、WAL / busy_timeout write connection、`BEGIN IMMEDIATE` transaction helper は実装済み。未実装は SQLite `events` table、既存 JSONL migration、state+event 1 transaction write command migration、export-only Markdown projection、SQLite events 由来の history / write-diff / revert-event。file-back skill cutover はこの substrate と native recovery が先に通ってから。
