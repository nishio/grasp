...
#### 2026-06-25: corpus を grasp 自身の wiki から wiki森全体（40+ wiki）へ広げる

出典: [[scrapbubble]] ingest 由来の設計対話（2026-06-25, nishio）。dogfood corpus を grasp 1 wiki から **親 llm-wiki の wiki森全体（home 配下 40+ の単一所有者 LLM Wiki、registry は `wiki_search.py` の `wikis.yaml`）**へ拡張する候補。

- **動機 = 森を (a-1) grep から (a-2) graph へ**: 森の現状の横断手段は親 llm-wiki の `wiki_search.py`（grep 横断）止まり＝**節点アクセス (a-1)**（「この語がどの wiki にあるか」）。**N wiki を跨いで参照されるが本文がどこにも無い概念**＝俯瞰グラフ層 **(a-2)** は出せない（親 llm-wiki でも「森は並存しているだけ／有機的 interaction は薄い」と観測されている）。grasp の whole-store cross-project（[[whole-store-graph-and-cross-project-edges]]）＋ Markdown mirror が、この (a-2) を森に供給できる。これは grasp が既に持つ「自分の wiki を dogfood」の自然な拡張＝**森が次の corpus**。
- **なぜ森が理想 corpus か（Co- 削ぎとの整合）**: cross-project 読みの価値は「**非 Co-（単一所有者が自分の複数 namespace を1グラフに統合）**」と「**Co-（多人数：他者の project を読む）**」の2層に割れる。grasp は Co-（多人数協調編集・他者 project 読み）を削ぐ（[[why-not-scrapbox-clone]]）ので継ぐのは前者だけ。**wiki森は全部 nishio 所有＝マクロな非 Co- 実例**で、grasp の cross-project がちょうど嵌まる（Co- 層がほぼ無い）。この2層分解は nishio が 2023 に Scrapbox プロジェクト間連携として既に分けていた軸の再来。
- **重要な含意 = 森用の特別 edge policy は要らない**（上の「どの記法を edge とみなすか」問題の解決, nishio 2026-06-25「import 時はバラバラ、その後 query 等で徐々に有機的に結合すればいい」）: 森の cross-wiki 参照（バックティックのプレーン名＝親 llm-wiki 概念への参照）を **import 時に edge へ materialize する判断は不要**。裸の **referenced-only 赤 node** のまま入れ、[[whole-store-graph-and-cross-project-edges]] point 8 の**弱い接続**（normalize-title の cross-project 一致＝著者が書いていない AI ヒント）が **query 時に繋ぐ**。「Markdown の束の束」は欠陥でなく、query 時結合を待つ正常な初期状態（親 llm-wiki `書いてから整理する` の森スケール版）。誤接続（同綴り別概念）は weak 層に封じ込まれ、人間が書いた強いグラフを汚さない。∴ mirror が森用に必要なのは新しい edge policy ではなく、**whole-store の weak 接続層（point 7+8）**だけ。
- **未実装 / 論点**:
  - 40+ wiki を 1 store の 40+ project namespace へ import するオーケストレーション（`wikis.yaml` を seed に、`import --markdown <folder> --project <wiki-name>` の一括実行）。
  - 森の navigation/log artifact 除外を森規模で効かせる（各 wiki の `index.md` / 親の `forest-index.md` / `wikis.yaml` は content graph 外＝下の「LLM Wiki index / navigation artifact handling」を森に適用）。
  - **raw/ 除外が性能前提**（親 llm-wiki 観測: `llm-wiki-about-nishio` は md 24,968 件、`kouchou-ai-developer-wiki` 2,000 件）。mirror import に raw/ 除外オプションが要る。
  - weak 接続の **cross-wiki spread ranking**（「何 wiki を跨いで参照されるか」を signal 化）＝森スケールの (a-2) の本体出力。
  - 森メタ側の対応物は親 llm-wiki に file back 済み（`wiki-forest-utilization-design-20260610` の Update）。grasp 側は本項が実装 backlog。
...
- **接続の強弱（nishio 2026-06-25 決定）**: `edges` に `connection_strength`（'strong' | 'weak'）を持たせる。**strong** = 人間が書いた明示リンク（intra `[X]` / explicit `[/P/T]`）。**weak** = grasp が normalize title の cross-project 一致で推論した接続（著者は書いていない＝AI 向けヒント）。bare 赤 `[X]` が他 project の materialized X に解決するのも weak（「自 project だけでは得られない content を他から発見」）。誤接続（同綴り別概念）は weak 層に閉じるので authored グラフを汚さない＝strength が point 7 の誤接続リスクの封じ込め機構。retrieval は strength を label し weak を strong より下に rank。`link_kind` や将来の typed/directional 軸とは直交。詳細は [[whole-store-graph-and-cross-project-edges]] point 8。
- **出力契約**: discover-broad-filter-post-hoc。relevance で pre-filter せず target_project / link_kind / connection_strength / scope ラベル付きで surface、絞りは post-hoc flag、出力量は rank + omitted-count で bound、性能は bound で対処し hide しない。現 `cross_project_refs` の `--semantic-only` / `--exclude-icons` / `--include-self` は「pre-filter」から「surface 済み集合への post-hoc filter」に位置づけ直す。
- **history**: store format / materialized index semantics が変わるため [[history]] の `x` を進める（再 import 要）。

未決（decision の Open Questions）: referenced-only namespace の coverage rollup surface 形 / slash-in-title 確定規則の実データ検証 / dense graph での whole-store related/path bound / weak 接続の rank・閾値調整と同綴り別概念の誤接続頻度の dogfood / link 同一判定の表記ゆれ吸収（`yyyy/MM/dd`⇄`yyyy-MM-dd` 等、[[scrapbubble]]）。赤-materialized 解決と explicit `[/P/T]` の strength 整合は point 8 で解決済み。
