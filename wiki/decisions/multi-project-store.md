...
### 2026-06-24: v6 全体決定が含意の2 clause を supersede（上の tentative Update と一部 diverge）

[[whole-store-graph-and-cross-project-edges]]（v6）が本 decision の含意 clause を覆す。**node を namespace ごとに分けて merge しない核は維持**し、変えるのは edge と default scope:
...
上の tentative Update（villagepump 由来）との **diverge を明示**（未解決、nishio 判断待ち）:

- tentative Update は「**resolved page graph は分離維持**（read/backlinks/related は別 project を混ぜない）」とした。v6 は逆に **resolved page も whole-store + label で接続**する（`villagepump/Keicho` を read すると `/nishio` が backlink に出る）。誤読回避を「分離」でなく「labeling」で達成する立場。
- tentative Update は「**別 project の同名赤リンク `[X]` を normalize 名 key で同一 node に束ねる**」とした。v6 はノードを `(project, norm)` で分離したまま保ち、**束ねず**、接続は明示 `[/P/T]` edge と whole-store labeling で行う（同名 bare 赤リンクの cross-project 統合は v6 には含めず Open Question として残す。[[whole-store-graph-and-cross-project-edges]] の Open Questions 参照）。
