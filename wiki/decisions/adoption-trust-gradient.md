...
- 信頼を「測る」指標は何か（recovery を何回成功したら mode2 に進めてよいか）。2026-06-30 の throwaway mode2 stress（[[mode2-parallel-edit-stress-2026-06-30]]）から、cutover gate の具体案が1つ出た: **「2プロセス並行 hot-page 奪い合いで lost-update 0 を実証する回帰試験が green」**。同 run は無協調 mode2 が silent lost-update を起こし（50中24消失）なお `write-status --strict` は GREEN になることを示し、整合チェックを指標にしてはいけないことも確定させた。前提として projection/graph compute の性能病（[[grasp-backlog]] P0a）が直っていること（直らないと grasp-write 自体が回らない）。2026-06-30 Codex follow-up 後、lost=0 の軽量 regression と claim retry benchmark harness は入ったが、mode2 cutover gate はまだ閉じていない。次に測るべきものは correctness だけでなく許容 overhead であり、broader / real dogfood workload で surviving throughput ratio、completed throughput ratio、p95 claim wait、strict green、active claim overlap 0 を並べ、owner が閾値を置くまでは mode2 は高信頼端 dogfood のままにする。
