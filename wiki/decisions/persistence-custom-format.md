...
## Update (2026-06-26): LLM Wiki cutover 後、Markdown は native store からの projection

[[native-authority-markdown-projection]] で、LLM Wiki を grasp infrastructure へ移す目標形を明確化した。既存 Markdown を読めることは import adapter の責務だが、cutover 後の authority は Markdown files ではなく native store（＋ durable journal）。Markdown は git review / backup / publish / interoperability のために出力し続ける **generated projection** になる。

したがって「Markdown を保存形式にしない」は「Markdown を捨てる」ではない。Markdown を authority から projection へ降ろし、`grasp write` が native store を更新して `grasp export-markdown` が `wiki/` を再生成する。
