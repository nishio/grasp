...
## [2026-06-30 23:37] implementation+file-back | measure log survival in throughput gate
- code: scripts/benchmark_claim_retry_throughput.py の file-back workload が append-log marker survival を読み戻し、lost_log_markers / log_lost column / log_lost_delta を出すようにした。
- measurement: --iterations 25 --think-seconds 0,0.02,0.05 --workload file-back --format table で、claim_retry は全3条件 page lost 0 / log_lost 0 / strict green / overlap 0。
- implication: synthetic file-back workload では append-log 喪失は観測されず、generic merge/queue を先に足す根拠はまだない。残る gate は owner の throughput/wait 閾値決定。

## [2026-06-30 23:42] implementation+file-back | add optional threshold gate to benchmark
- code: scripts/benchmark_claim_retry_throughput.py に --min-surviving-throughput-ratio / --max-p95-claim-wait-seconds を追加。owner が値を指定した時だけ optional cutover gate を JSON/table に出し、未達なら exit 1。
- verification: permissive thresholds は pass、厳しい thresholds は surviving_throughput_ratio_below_threshold / p95_claim_wait_above_threshold で fail。既存 claim retry / strict overwrite 回帰も green。
- policy: Codex は owner 閾値を決めない。mode2 cutover はこの2値を owner が置いてから判断する。

## [2026-06-30 23:58] file-back | mode2 Markdown readonly guard
- [[grasp-backlog]] に `scripts/check_mode2_markdown_readonly.py` を安定マイルストーンの mode2 Markdown 直接編集 gate として記録。Markdown drift は既定 reject、意図的 fallback / remote merge は `reconcile-markdown --dry-run` → fresh `GRASP_SESSION_ID` で明示 reconcile、generic merge / queue は実需要まで保留。

## [2026-07-01 00:13] implementation+file-back | Markdown re-import line id inheritance
- code: Markdown content-only re-import が旧新 lines の exact diff で同一 line の stored `line_id` を引き継ぐようにした。挿入で positional candidate が既存 id と衝突する新規 line は opaque suffix を mint し、self / incoming anchor の `target_line_id` も inherited id を指す。
- coverage: fast changed-file path と full-parse incremental path の両方、挿入後の line_id 生存、self anchor target_line_id 再マップを regression test に追加。`python3 -m unittest discover -s tests` は 330 tests OK。
- caveat: stable line identity はまだ完了ではない。Cosense / hosted sync、line tombstone、line-id 指定 write、split / merge / move / ambiguous duplicate line policy は [[grasp-backlog]] に残る。
