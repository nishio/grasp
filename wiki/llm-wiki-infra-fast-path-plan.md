...
2026-06-26 update: `1.7.37` で同 harness に `revert_events` step を追加し、`1f8274c` の fast-path plan page create を `event_revert` で取り消し、既存3 page update は残す sequence を replay clean にした。created page の absent invariant と残存 page exact projection を同時に見る。
