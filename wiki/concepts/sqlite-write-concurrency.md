...
- **耐久層 thesis は同 session で自己実証された**: 上記「耐久層の所在」の考察を file back する作業中に、前述の git-layer interleave（line 81）が実際に起き、考察を書いた当の Claude Code session の uncommitted hunk が別 agent の commit `84c6d7c` に吸われた。だが hunk は失われず、その commit は **PR #44 経由で origin/main に merge** された。私自身は一度も成功 commit していないのに、git commit という耐久層が hunk を main まで運んだ ―「working-tree は clobberable / git commit が耐久層」を、その主張を書いた session 自身が身をもって裏づけた。lock 管轄外の git 層で clobber されかけても、commit された時点で耐久層に入る、が実地で確認できた。
...
## Updates 2026-06-29
- **SQLite SSoT は Git merge authority ではない**（live Q: 「SQLite SSoT なのになぜ Markdown が conflict するのか」）。repo-local file-back の現在形では `.grasp/file-back.sqlite` が live write authority で、`wiki/*.md` は git-tracked projection / review snapshot、かつ `.grasp/file-back.sqlite` は gitignored。従って `write-page` / `append-log` が同じ store に直列実行される範囲では SQLite SSoT が効くが、branch merge / `git merge --autostash` は SQLite events を見ず、tracked `wiki/log.md` の text を merge する。Git には「両側の `log_append` event を union して projection を再生成する」という意味情報が無い。
- **今回の bad experience は P3 AI author/file-back agent の friction**: `codex/persona-emulation-plan` を dirty primary `main` へ `git merge --ff-only --autostash` したところ、merge 自体は fast-forward したが autostash replay が `wiki/log.md` 末尾 append で conflict した。正しい復旧は `11:51` persona2a demo log、`12:19` persona queue、`12:23` success shape を全て残す append union だった。data loss ではないが、ユーザには「merge 済み / dirty 継続 / autostash 残存 / 手 union 必須」が分かりにくく、体験として重い。
- **改善方向**: Git merge まで SQLite SSoT に揃えるなら、(a) events-aware merge（両 branch の SQLite events を union → projection regenerate）、(b) git-tracked durable event bundle、(c) `log.md` projection を merge 対象から外す/遅延 batch する、のいずれかが必要。現状の practical recovery は conflict marker を消して append union、`rg '<<<<<<<|=======|>>>>>>>'` / `lint_wiki.py` / `git diff --check` で検証し、autostash が残るなら「安全コピーなのか未適用なのか」を明示すること。

## Updates 2026-08-31
- **busy_timeout / WAL は既に在る (line 35 の「皆無」は 1.7.39 で解消済み)**: `connect_sqlite_store(for_write=True)` は `PRAGMA busy_timeout=30000`(30s) + `journal_mode=WAL` を設定し、write は `sqlite_write_transaction` の `BEGIN IMMEDIATE` で直列化する（[sqlite_store.py:253-296](grasp/sqlite_store.py#L253-L296)、`BEGIN IMMEDIATE` は per-write-tx であって connect 時でも fetch 中でもない）。∴ 同一 store への 2nd writer は即 fail でなく **30s 待ってから** fail する。line 35 の分析は 2026-06-26 incident 当時の状態として読む。
- **だが大きな namespace-replace write は 30s busy_timeout を超えうる**: `acquire` / `sync` の store mutation は `replace_project_with_cosense_pages` → `_delete_project`(6 テーブル DELETE) + 全ページ re-INSERT を **1 write transaction** で行う。大規模 project（例 20000p 指定の外部 project）ではこの write が 30s を超え、WAL も肥大（実測 155MB）するため、共有 default store `~/.grasp/grasp.sqlite` への並行 writer が `database is locked` で落ちる。Open Q（line 62）「default store を複数 agent が同時使用したら」の実地確認（2026-08-28、別 Claude session の外部 project acquire chain と衝突）。
- **acquire は lock 負けで fetch 済みを全喪失する**: `acquire` は候補を全て in-memory に fetch してから最後に 1 回 replace-write する構造なので、その最終 write が busy_timeout 超過で落ちると **数百ページ・数時間分の fetch を 1 件も残さず捨てる**（今回その 862p project がこれで 2 回喪失）。回避は他 writer の完了を待って再 acquire（待機後に 862p 完走）。fix 候補: mutation path に write-retry / より長い busy_timeout / 別 store で fetch してから merge。
- cf. [[cosense-fetch-rate-limit]] — この lock 待ちと直交して hosted 側の 429 rate-limit も別途効く。
