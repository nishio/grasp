...
- **だが大きな namespace-replace write は 30s busy_timeout を超えうる**: `acquire` / `sync` の store mutation は `replace_project_with_cosense_pages` → `_delete_project`(6 テーブル DELETE) + 全ページ re-INSERT を **1 write transaction** で行う。大規模 project（例 ある private project 20000p）ではこの write が 30s を超え、WAL も肥大（実測 155MB）するため、共有 default store `~/.grasp/grasp.sqlite` への並行 writer が `database is locked` で落ちる。Open Q（line 62）「default store を複数 agent が同時使用したら」の実地確認（2026-08-28、別 Claude session の 外部 project acquire chain と衝突）。
- **acquire は lock 負けで fetch 済みを全喪失する**: `acquire` は候補を全て in-memory に fetch してから最後に 1 回 replace-write する構造なので、その最終 write が busy_timeout 超過で落ちると **数百ページ・数時間分の fetch を 1 件も残さず捨てる**（今回 862p の private project がこれで 2 回喪失）。回避は他 writer の完了を待って再 acquire（待機後に 862p 完走）。fix 候補: mutation path に write-retry / より長い busy_timeout / 別 store で fetch してから merge。
