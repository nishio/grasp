...
| 層 | 状態 | 内容 |
|---|---|---|
| **read** | 既存に近い | `mentions <query>` ＝ 用語の仮想出現一覧。come-from の「集まった姿」を読む側。[[grasp-backlog]] の `mentions --unlinked` / `co-links` がここ |
...
## 6. 裸言及には3系統ある（`mentions --unlinked` の設計）

[[grasp-backlog]] は現状「link gap か 意図的 non-link か」の2分で `mentions --unlinked` を設計している。今回の audit で第3が判明した:
...
実例: KJ法 audit でトップの bare mention page `🌀KJ法`（266 occurrences）は **AI 作ページ**で (c) だった。**書き手が AI 化するほど (c) が支配的**になり、裸言及は著者の意図と無関係に増える。∴ `mentions --unlinked` の出力は「埋めるべき gap」だけでなく **「come-from 昇格候補」（高頻度・uncommon・一意の裸語）** を別枠で出すべき。目的は bulk link 化でなく、(a)(b)(c) を見分けて「1宣言で畳めるもの」を surface すること。
...
- `mentions <query> --unlinked`: (a)(b)(c) を分け、come-from 昇格候補（uncommon×頻度×一意）を別枠で返す（[[grasp-backlog]] の既存 `mentions` 候補を拡張）。
...
- come-from 昇格候補の閾値（uncommon さ・頻度・一意性）をどう機械化するか。`mentions --unlinked` の出力に scoring を載せるか。
