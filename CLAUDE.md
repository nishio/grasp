...
file back は **grasp write first**。通常編集の authority は SQLite store 側に置く。tracked `wiki.grasp/events.jsonl` は `1.8.18` で退役・削除済みで、通常 file-back は repo に JSONL を作らない。まず各 file-back に一意な `GRASP_SESSION_ID`（例: `file-back-YYYYMMDDTHHMMSSZ-topic`）を export し、`git fetch origin` 後に `python3 scripts/check_file_back_preflight.py`（no-journal default）を走らせる。preflight は current upstream（なければ `origin/main`）を基準に、未使用 session id を要求し、remote 分岐なし / wiki dirty なし / 退役済み JSONL path の再作成なし / `write-status --no-journal --strict` / SQLite authority projection を確認し、gitignored preflight stamp に session/head/base を記録する。その後、`append-section` / `write-page` / `append-log` を `--no-journal --output wiki` 付きで使う。postwrite は同じ session id を要求し、write 後に `python3 scripts/check_file_back_postwrite.py`（no-journal default）で preflight stamp の session/head/base 一致 / latest SQLite event の `session_id` / no-journal strict status / SQLite authority projection / SQLite events 由来の semantic log projection / wiki lint / whitespace diff check を確認する。これが通らない時、または任意 frontmatter merge / canonical docs など grasp alpha が表現できない時だけ direct Markdown patch に fallback し、理由を log に残す。
