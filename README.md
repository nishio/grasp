...
書き込み（`write` / `rename`）は v1 には含まれません（[Roadmap](#roadmap) 参照）。Markdown mirror の最小版は `grasp import --markdown <folder>` で使えます。frontmatter `title` / `id` / `aliases` / `tags` を読み、title が無い場合は first H1、さらに無ければ file stem を title とし、`[[wikilink]]` と `#tag` を edge として index します。
