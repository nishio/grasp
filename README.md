...
この repo の開発 wiki dogfood では、repo-local file-back guard scripts は no-journal が default です。tracked `wiki.grasp/events.jsonl` は `1.8.18` で退役済みで、通常 file-back は repo に JSONL を作りません。各 file-back は `GRASP_SESSION_ID` を設定し、postwrite は同じ session id を要求して SQLite events 由来の semantic log projection も確認します。
...
- repo-local file-back guard scripts は no-journal が default で、postwrite は同じ session id を要求し、SQLite events 由来の semantic log projection も確認し、compatibility journal あり mode は明示 audit 用に残す
