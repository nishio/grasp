...
本文に加えて、逆リンク（行つき）・related ページ・未解決ターゲットがまとめて返ります。人間が出力フォーマットを覚える前提ではないので、詳細は使う直前の `grasp read --help` と、機械可読に見るための `grasp --json read "<ページタイトル>"` に寄せます。
...
- **書き込み層（`write` / `rename`）** — `[[X]]` を書けば逆リンクが自動で立つ。`rename` してもリンクが id を指すので**切れず、参照文の文意も汚れない**（名前＝識別子という前提を直す）。
