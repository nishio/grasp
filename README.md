...
汎用の `write` / replayable native authority は v1 の stable surface には含まれません（[Roadmap](#roadmap) 参照）。ただし Markdown-backed project の fast-path dogfood 用に、`append-section` / `append-log` / `write-page` / `rename-page` の authoring alpha があります。Markdown mirror の最小版は `grasp import --markdown <folder>` で使えます。frontmatter `title` / `id` / `aliases` / `tags` を読み、title が無い場合は first H1、さらに無ければ file stem を title とし、`[[wikilink]]` と `#tag` を edge として index します。
