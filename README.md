...
書き込み（`write` / `rename`）は v1 には含まれません（[Roadmap](#roadmap) 参照）。Markdown mirror の最小版は `grasp import --markdown <folder>` で使えます。file stem を title とし、`[[wikilink]]` と `#tag` を edge として index します。
...
最小版の title は file stem です。既存ファイルへは書き戻しません。
...
本文に加えて、逆リンク（行つき）・related ページ・未解決ターゲットがまとめて返ります。related ページの冒頭も同じ出力で見たい時は `grasp read "<ページタイトル>" --related-snippets` を使います。人間が出力フォーマットを覚える前提ではないので、詳細は使う直前の `grasp read --help` と、機械可読に見るための `grasp read "<ページタイトル>" --json` に寄せます。
...
| verb | 用途 |
| --- | --- |
| `read <title>` | 本文＋逆リンク＋related＋未解決を**近傍同梱**で返す（`--related-snippets` で related/source ページ冒頭も同梱） |
| `search <query>` | 本文行を検索し、単一語は行 substring、空白区切り複数語は page 単位 AND。0件時は NFKC/長音ゆれの normalized fallback を試す |
...
- **Markdown / Obsidian 互換性の拡張** — 最小の read-only mirror は実装済み。frontmatter title / aliases / duplicate resolution / Obsidian block refs などは今後の拡張。
