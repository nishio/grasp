...
この repo の開発 wiki dogfood では、repo-local file-back guard scripts は no-journal が default です。tracked `wiki.grasp/events.jsonl` は `1.8.18` で退役済みで、通常 file-back は repo に JSONL を作りません。各 file-back は一意な `GRASP_SESSION_ID` を設定し、preflight は current upstream（なければ `origin/main`）を基準に未使用 session id を要求し、gitignored preflight stamp に session/head/base を記録します。最初の write command 直前には write-start checker が import なしで projection / stamp / store status がまだ clean か確認し、postwrite は同じ session id を要求して preflight stamp の session/head/base 一致と SQLite events 由来の semantic log projection も確認します。ship loop は commit 後・push 前に `python3 scripts/check_push_ownership.py` を通し、dirty worktree / behind branch / 通常の protected branch push を止めます。
...
- repo-local file-back guard scripts は no-journal が default で、preflight は未使用 session id を要求して gitignored preflight stamp に session/head/base を記録し、write-start checker は import なしで projection / stamp / store status を確認し、postwrite は同じ session id を要求して preflight stamp の session/head/base 一致と SQLite events 由来の semantic log projection も確認し、compatibility journal あり mode は明示 audit 用に残す
