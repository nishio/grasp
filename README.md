...
- repo-local file-back guard scripts は no-journal が default で、preflight は未使用 session id を要求して gitignored preflight stamp に session/head/base を記録し、default store/output pair と temp store + temp output の混在を止め、write-start checker は import なしで projection / stamp / store status を確認し、postwrite は同じ session id を要求して preflight stamp の session/head/base 一致と SQLite events 由来の semantic log projection も確認し、compatibility journal あり mode は明示 audit 用に残す
