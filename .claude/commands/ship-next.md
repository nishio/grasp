...
2. File back the useful facts into the development wiki before committing:
   ...
   - for grasp-write-backed file back, first set one per-file-back `GRASP_SESSION_ID`, optionally create same-session active `page_claim` events with `claim-page`, then run `git fetch origin` and `python3 scripts/check_file_back_preflight.py` (no-journal default); preflight uses the current upstream branch, origin/<branch>, or HEAD for no-upstream feature continuation as its base, falls back to `origin/main` for protected branches, requires an unused session id except for active same-session `page_claim` events created before preflight, fresh repo store bootstraps through gitignored `.grasp/file-back-adopt.jsonl`, writes a gitignored preflight stamp with session/head/base and latest SQLite event_sequence, and acquires gitignored file-back lock `.grasp/file-back.lock.json`.
   - use the repo default store/output pair `.grasp/file-back.sqlite` + `wiki`; temporary dogfood must use a temporary store and temporary output, never the repo store with a temporary output.
   - immediately before the first write command, run `python3 scripts/check_file_back_write_start.py` (no-journal default) to verify the projection, stamp, lock, and store status are still clean, and that the latest SQLite event_sequence has not changed since preflight, without re-importing Markdown.
