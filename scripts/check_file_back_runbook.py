...
DEFAULT_RULES: tuple[RunbookRule, ...] = (
    ...
    RunbookRule(
        ...
        required=(
            ...
            "latest SQLite event_sequence",
            "write-start checker",
            "import なしで projection / stamp / store status",
            "preflight stamp の session/head/base 一致",
            "postwrite は同じ session id を要求",
            "preflight 後に増えた全 SQLite events",
            "python3 scripts/check_push_ownership.py",
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print("file-back runbook ok: no-journal default, retired journal, semantic log guard, session window marker, preflight stamp, write-start guard, store/output pair guard, and push ownership guard documented")
    return 0
