...
DEFAULT_RULES: tuple[RunbookRule, ...] = (
    RunbookRule(
        ...
        required=(
            ...
            "`GRASP_SESSION_ID`",
            "current upstream / `origin/<branch>` / no-upstream feature `HEAD`（protected branch は `origin/main`）",
            "未使用 session id を要求",
            "active `page_claim`",
    ...
    RunbookRule(
        ...
        required=(
            ...
            "`GRASP_SESSION_ID`",
            "preflight uses the current upstream branch, origin/<branch>, or HEAD for no-upstream feature continuation as its base",
